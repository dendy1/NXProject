...
namespace NXProject
{
    public class SessionDrawer
    {
        ...
        {
            Session.Part.New(filename, 1, out _);
        }
        ...
        public Tag DrawLine(Point startPoint, Point endPoint)
        {
            ...
            Tag sketchPart;
            Session.Curve.CreateLine(ref line, out sketchPart);
        ...
        public Tag DrawArc(double startAngle, double endAngle, double radius, Point center)
        {
            ...
            Session.Csys.AskWcs(out wcs_tag);
            Session.Csys.AskMatrixOfObject(wcs_tag, out matrix_tag);
            arc.matrix_tag = matrix_tag;
            Tag sketchPart;
            Session.Curve.CreateArc(ref arc, out sketchPart);
            return sketchPart;
        }
        ...
        public Tag DrawArcRotX(double startAngle, double endAngle, double radius, double angle, Point center)
        {
            ...
            Session.Mtx3.RotateAboutAxis(rotAxis, angle, matrix);
            arc.arc_center = new double[3];
            Session.Mtx3.VecMultiply(center.Array, matrix, arc.arc_center);
            Session.Csys.CreateMatrix(matrix, out arc.matrix_tag);
            arc.start_angle = startAngle;
            arc.end_angle = endAngle;
            arc.radius = radius;
        ...
        {
            Tag[] output;
            Session.Modl.CreateExtruded(sketch, taperAngle, limit, new double[3], direction, sign, out output);
            return output;
        }
        ...
        {
            Tag[] output;
            Session.Modl.CreateRevolved(sketch, limit, refPoint.Array, direction, sign, out output);
            return output;
        }
        ...
        {
            Tag output;
            Session.Modl.CreateChamfer(3, offset.ToString(CultureInfo.InvariantCulture), offset.ToString(CultureInfo.InvariantCulture), angle.ToString(CultureInfo.InvariantCulture), tags, out output);
            return output;
        }
        ...
        {
            Tag output;
            Session.Modl.CreateBlend(radius.ToString(CultureInfo.InvariantCulture), tags, 0, 0, 0, 0.0, out output);
            return output;
        }
        ...
        public void MakeThread(double majorDiameter, double minorDiameter, double diameter, double[] direction, double length, double step, Tag cylFace, Tag startFace)
        {
            ...
            threadData.angle = "60";
            Session.Modl.CreateSymbThread(ref threadData, out output);
        }
    }
}
