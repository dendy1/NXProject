...
namespace NXProject
{
    public partial class DetailsForm : Form
    {
        ...
        {
             new Bolt("M10", 7,  25, 10, 40,  10, 30, 0.6, 1.5),
            ...
            try
            {
                switch (detailsTabControl.SelectedIndex)
                {
                    case 0:
                        ...
                        bolt.Draw(UFSession.GetUFSession(), path + "bolt" + System.IO.Directory.EnumerateFiles(path).Where(f => f.Contains("bolt")).ToList().Count);
                        break;
                    case 1:
                        var screw = Screws[screwComboBox.SelectedIndex];
                        screw.Length = double.Parse(screwLengthComboBox.SelectedItem.ToString());
                        screw.Draw(UFSession.GetUFSession(), path + "screw" + System.IO.Directory.EnumerateFiles(path).Where(f => f.Contains("screw")).ToList().Count);
                        break;
                    case 2:
                        var nut = Nuts[nutComboBox.SelectedIndex];
                        nut.Draw(UFSession.GetUFSession(), path + "nut" + System.IO.Directory.EnumerateFiles(path).Where(f => f.Contains("nut")).ToList().Count);
                        break;
                    default:
                        break;
                }
